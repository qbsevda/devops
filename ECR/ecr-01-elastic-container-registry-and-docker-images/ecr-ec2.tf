...
resource "aws_iam_role" "ec2ecrfullaccess" {
  ...
  managed_policy_arns = ["arn:aws:iam::aws:policy/AmazonEC2ContainerRegistryFullAccess"] # burada Iam role var (butun yetkileri ver anlaminda sanirsam)
...
resource "aws_instance" "ecr-instance" {
  ...
  key_name        = "sysy" # you need to change this line
...
output "ssh-connection" {
  value = "ssh -i ~/Desktop/Keys/sysy.pem ec2-user@${aws_instance.ecr-instance.public_ip}"
}